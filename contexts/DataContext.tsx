...
interface DataContextType {
  ...
  updateEnrollmentProgress: (enrollmentId: string, progress: number, completedLessons: string[]) => void
...
export const DataProvider: React.FC<DataProviderProps> = ({ children }) => {
  ...
  const updateEnrollmentProgress = (enrollmentId: string, progress: number, completedLessons: string[]) => {
    setEnrollments((prev) =>
      prev.map((e) =>
        e.id === enrollmentId
          ? { ...e, progress, completedLessons, status: progress === 100 ? "completed" : "active" }
          : e,
      ),
    )
  }
